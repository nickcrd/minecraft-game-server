...
package com.nickcontrol.arcade.game.games.gladiators.kits;
...
{
    public GladiatorKit() {
        super("Gladiator", KitAvailability.FREE, 0);
    }
    ...
    public void applyKit(GamePlayer player)
    {
        ...
        player.getPlayer().getInventory().setArmorContents(new ItemStack[]{
                new ItemBuilder(Material.CHAINMAIL_BOOTS).build(), new ItemBuilder(Material.CHAINMAIL_LEGGINGS).build(), new ItemBuilder(Material.CHAINMAIL_CHESTPLATE).build(), new ItemBuilder(Material.CHAINMAIL_HELMET).build()});
        player.getPlayer().getInventory().addItem(new ItemBuilder(Material.GOLDEN_APPLE).setAmount(2).build());
    }
}
